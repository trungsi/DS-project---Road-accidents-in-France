...
def train(df, estimator, scaler, target_col='grav_mean'):
  ...
  train_result['errors'] = np.absolute(y_test_pred - y_test)
...
def plot_results_histogram(train_result):
    fig, axes = plt.subplots(1, 2, figsize=(10, 5))
    sns.histplot(train_result['y_test'], bins=12, ax=axes[0]).set(title='Real values')
    sns.histplot(train_result['y_test_pred'], bins=12, ax=axes[1]).set(title='Predicted values')
    return fig
...
def plot_results_scatter_chart(train_result):
    ...
    plt.plot(train_result['errors'], color='green')
